...
<div class="container-fluid">
    ...
    <div class="container">
        ...
        if ($datos !== null) {
            ...
            echo "<h1 class='cultural'><b> ACTIVIDADES CULTURALES </b></h1>";
            echo "<thead><tr>
                    <th>ID</th>
                    <th>Actividad</th>
                    <th>Instructor</th>
            ...
            echo "<tbody>";
            foreach ($culturales as $cultural) {
                echo "<tr id='fila{$cultural['id']}'>
                        <td>{$cultural['id']}</td>
                        <td>{$cultural['actividad']}</td>
                        <td>{$cultural['instructor']}</td>
                        <td>{$cultural['lunes']}</td>
                        <td>{$cultural['martes']}</td>
                        <td>{$cultural['miercoles']}</td>
                        <td>{$cultural['jueves']}</td>
                        <td>{$cultural['viernes']}</td>
                        <td class='text-center'>
                            <div class='d-flex justify-content-center'>
                                <button class='btn btn-ver' onclick='verDetalles(". json_encode($cultural) .")'>Ver</button>
                                <a href='actualizarCultural.php?id={$cultural['id']}' class='btn btn-modificar'>Modificar</a>
                                <form method='post' action='eliminar_cultural.php' id='formEliminar{$cultural['id']}'>
                                    <input type='hidden' name='id' value='{$cultural['id']}' />
                                    <button type='button' class='btn btn-eliminar' onclick='confirmarEliminacion({$cultural['id']})'>Eliminar</button>
                                </form>
                            </div>
                        </td>
                      </tr>";
        ...
        ?>
        <br>
        <a href="agregarCultural.php" class="btn btn-agregar">Agregar</a>
        <br>
        ...
        function verDetalles(cultural) {
            ...
            detallesActividad.innerHTML = '<p><strong>ID:</strong> ' + cultural.id + '</p>' +
                                          '<p><strong>Actividad:</strong> ' + cultural.actividad + '</p>' +
                                          '<p><strong>Instructor:</strong> ' + cultural.instructor + '</p>' +
                                          '<p><strong>Lunes:</strong> ' + cultural.lunes + '</p>' +
                                          '<p><strong>Martes:</strong> ' + cultural.martes + '</p>' +
                                          '<p><strong>Miércoles:</strong> ' + cultural.miercoles + '</p>' +
                                          '<p><strong>Jueves:</strong> ' + cultural.jueves + '</p>' +
                                          '<p><strong>Viernes:</strong> ' + cultural.viernes + '</p>';
